...
// ====== Touch 轨迹纹理(liquid 扭曲的采样源)======
// 官方 demo github.com/zavalit/bayer-dithering-webgl-demo 的 liquid 输入
...
// ====== Liquid 扭曲 Effect(仅改 UV,postprocessing 默认采样 inputBuffer)======
...
const buildInstance = (container: HTMLElement, cfg: PixelBlastConfig): PixelBlastInstance => {
  const {
    variant,
    pixelSize,
    color,
  ...
  let raf = 0;
  const animate = () => {
    if (autoPauseOffscreen && hidden.value) {
...
const disposeInstance = (instance: PixelBlastInstance): void => {
  instance.resizeObserver?.disconnect();
  cancelAnimationFrame(instance.raf);
  instance.timer.dispose();
  instance.quad?.geometry.dispose();
  instance.material.dispose();
  instance.composer?.dispose();
  instance.touch?.texture.dispose();
  instance.renderer.dispose();
  instance.renderer.forceContextLoss();
  if (instance.renderer.domElement.parentElement) {
    instance.renderer.domElement.parentElement.removeChild(instance.renderer.domElement);
  }
};
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const instance = buildInstance(container, {
      variant,
      pixelSize,
      color,
      antialias,
